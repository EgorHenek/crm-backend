...
RSpec.describe "News", type: :request do
  ...
  describe 'GET /news/:id' do
    ...
    it 'return 200 with created_at and updated_at' do
      get news_path(@news.slug), headers: auth_headers(@advertising)
      expect(response).to have_http_status(200)
    ...
    describe 'Валидация' do
      ...
      it 'Попытка получить доступ к неопубликованой новости с правами' do
        get news_path(@un_published_news.slug), headers: auth_headers(@advertising)
  ...
  describe 'POST /news' do
    it 'return created' do
      post news_index_path, params: attributes_for(:news), headers: auth_headers(@advertising)
  ...
  describe 'DELETE /news/:id' do
    it 'return no_content' do
      delete news_path(@news.slug), headers: auth_headers(@advertising)
  ...
  describe 'PUT /news/:id' do
    it 'return 200' do
      put news_path(@news.slug), headers: auth_headers(@advertising), params: { title: 'new title' }
      expect(response).to have_http_status(:ok)
      expect(response).to match_json_schema('news/id_with_created_at')
    end
